...
public class MessageParserUtil {
	...
	private static <T extends MessageBuilder> void addColouredContent(String msg, T builder, Pattern colourPattern) {
		int previous = 0;
		Matcher colourMatcher = colourPattern.matcher(msg);
		while (colourMatcher.find()) {
			String content = msg.substring(previous, colourMatcher.start());
			builder.add(content);
			Format format = Format.fromCode(Character.toLowerCase(colourMatcher.group().charAt(1)));
	...
	private static Message parse(String msg, Pattern processor) {
		MessageBuilder builder = new SimpleMessageBuilder();
		addColouredContent(msg, builder, processor);
	 ...
	 */
	public Message parse(String msg, FormattingCodePattern colourPattern) {
		return parse(msg, colourPattern.getPattern());
	}
	 ...
	 * @return a formed Message
	 */
	public Message parseUncoloured(String msg) {
	...
	private static Message parseJson(String msg, Pattern processor) {
		return parseJson(msg, (node, builder) -> addColouredContent(node, builder, processor), (tooltip) -> parse(tooltip, processor));
	}
	 ...
	 * @return a formed Message
	 */
	public Message parseJson(String msg, FormattingCodePattern colourPattern) {
		return parseJson(msg, colourPattern.getPattern());
	}
	 ...
	 * @return a formed Message
	 */
	public Message parseJson(String msg) {
	 ...
	 * @return a formed Message
	 */
	public Message parseUncolouredJson(String msg) {
