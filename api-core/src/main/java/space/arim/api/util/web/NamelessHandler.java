...
public class NamelessHandler {
	...
	private boolean postParams(String action, String...parameters) throws HttpStatusException, SenderException {
		try (SenderConnection conn = new SenderConnection(baseUrl + action)) {
			...
			return !conn.response().containsKey("error") || !((Boolean) conn.response().get("error"));
