...
@Deprecated
class BungeeComponentParser implements PlatformFormattingParser<BaseComponent[]> {

	@Override
	public BaseComponent[] colour(String msg) {
		return colourProcessor(msg, ColourPatterns.COLOUR_PATTERN);
	}
	...
	@Override
	public BaseComponent[] parseJson(String msg) {
		Pattern pattern = ColourPatterns.COLOUR_PATTERN;
	...
	private static BaseComponent[] parseJsonProcessor(String msg, Function<String, TextComponent> nodeGenerator,
		...
		for (String node : ColourPatterns.DOUBLE_PIPES.split(msg)) {
			if (node.isEmpty()) {
				continue;
			}
			...
			if (tag == JsonTag.NONE) {
				...
				switch (tag) {
				case TTP:
					HoverEvent hoverEvent = new HoverEvent(HoverEvent.Action.SHOW_TEXT, tooltipGenerator.apply(value));
